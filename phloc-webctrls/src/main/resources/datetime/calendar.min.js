...
b=this.date.getHours(),c=this.date.getMinutes();d&&((a=12<=b)&&(b-=12),0===b&&(b=12),s.innerHTML=a?"pm":"am");k.innerHTML=10>b?"0"+b:b;q.innerHTML=10>c?"0"+c:c};g.onUpdateTime=function(){var a=this.date,b=parseInt(k.innerHTML,10);d&&(/pm/i.test(s.innerHTML)&&12>b?b+=12:/am/i.test(s.innerHTML)&&12==b&&(b=0));var c=a.getDate(),e=a.getMonth(),f=a.getFullYear();a.setHours(b);a.setMinutes(parseInt(q.innerHTML,10));a.setFullYear(f);a.setMonth(e);a.setDate(c);this.dateClicked=!1;this.callHandler()}}()):
this.onSetTime=this.onUpdateTime=function(){};a=Calendar.createElement("tfoot",a);f=Calendar.createElement("tr",a);f.className="footrow";e=c(Calendar._TT.SEL_DATE,this.weekNumbers?8:7,300);e.className="ttip";this.isPopup&&(e.ttip=Calendar._TT.DRAG_TO_MOVE,e.style.cursor="move");this.tooltips=e;this.monthsCombo=c=Calendar.createElement("div",this.element);c.className="combo";for(d=0;d<Calendar._MN.length;++d)a=Calendar.createElement("div"),a.className=Calendar.is_ie?"label-IEfix":"label",a.month=d,
a.innerHTML=Calendar._SMN[d],c.appendChild(a);this.yearsCombo=c=Calendar.createElement("div",this.element);c.className="combo";for(d=12;0<d;--d)a=Calendar.createElement("div"),a.className=Calendar.is_ie?"label-IEfix":"label",c.appendChild(a);this._init(this.firstDayOfWeek,this.date);b.appendChild(this.element)};
...
case "%P":case "%p":/pm/i.test(g[h])&&12>n?n+=12:/am/i.test(g[h])&&12<=n&&(n-=12);break;case "%M":p=parseInt(g[h],10)}isNaN(d)&&(d=c.getFullYear());isNaN(e)&&(e=c.getMonth());isNaN(f)&&(f=c.getDate());isNaN(n)&&(n=c.getHours());isNaN(p)&&(p=c.getMinutes());if(0!==d&&-1!==e&&0!==f)return new Date(d,e,f,n,p,0);d=0;e=-1;for(h=f=0;h<g.length;++h)if(-1!=g[h].search(/[a-zA-Z]+/)){k=-1;for(m=0;12>m;++m)if(Calendar._MN[m].substr(0,g[h].length).toLowerCase()==g[h].toLowerCase()){k=m;break}-1!==k&&(-1!==e&&
