...
d){var a,c;for(c in d)a=d[c],this.patternProp.test(c)?$("#"+b+" [name="+c.substring(0,c.length-2)+"]").prop("checked",a):this.patternArray.test(c)?$("#"+b+" [name="+c.substring(0,c.length-2)+"]").val(a):$("#"+b+" [name="+c+"]").val(a)},updateElementDirect:function(b,d){$("#"+b).empty().append(d)},updateElementViaAjax:function(b,d){$.ajax({url:d,success:function(a){a.success&&a.value&&FormHelper.updateElementDirect(b,a.value.html)}})},updateElements:function(b){for(var d in b){var a=b[d];a.url?FormHelper.updateElementViaAjax(a.id,
a.url):FormHelper.updateElementDirect(a.id,a.html)}},saveFormData:function(b,d,a,c,e,f){b=FormHelper.getAllFormValues(b,d);b.$pageID=a;$.ajax({url:c,data:b,success:function(a){a.success?FormHelper.updateElements(e):FormHelper.updateElements(f)}})},setSelectOptions:function(b,d){b.empty();$.each(d,function(a,c){b.append($("<option></option>").attr("value",a).text(c))})}};$.ajaxSetup({cache:!1});var FormHelper=window.FormHelper=new FormHelperClass;
