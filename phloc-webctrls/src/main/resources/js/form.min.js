...
d){var a,c;for(c in d)a=d[c],this.patternProp.test(c)?$("#"+b+" [name="+c.substring(0,c.length-2)+"]").prop("checked",a):this.patternArray.test(c)?$("#"+b+" [name="+c.substring(0,c.length-2)+"]").val(a):$("#"+b+" [name="+c+"]").val(a)},updateElementDirect:function(b,d){$("#"+b).empty().append(d)},updateElementViaAjax:function(b,d){$.ajax({url:d,success:function(a){a.success&&FormHelper.updateElementDirect(b,a.value.html)}})},updateElements:function(b){for(var d in b){var a=b[d];a.url?FormHelper.updateElementViaAjax(a.id,
