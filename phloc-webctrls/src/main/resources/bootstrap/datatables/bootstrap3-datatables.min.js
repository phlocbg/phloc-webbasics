...
$.extend($.fn.dataTableExt.oPagination,{bootstrap:{fnInit:function(b,e,c){var d=b.oLanguage.oPaginate,a=function(a){a.preventDefault();b.oApi._fnPageChange(b,a.data.action)&&c(b)};$(e).append('<ul class="pagination"><li class="prev disabled"><a href="#">&larr; '+d.sPrevious+'</a></li><li class="next disabled"><a href="#">'+d.sNext+" &rarr; </a></li></ul>");e=$("a",e);$(e[0]).bind("click.DT",{action:"previous"},a);$(e[1]).bind("click.DT",{action:"next"},a)},fnUpdate:function(b,e){var c=b.oInstance.fnPagingInfo(),
d=b.aanFeatures.p,a,g,k,f,h;a=Math.floor(2.5);5>c.iTotalPages?(f=1,h=c.iTotalPages):c.iPage<=a?(f=1,h=5):c.iPage>=c.iTotalPages-a?(f=c.iTotalPages-5+1,h=c.iTotalPages):(f=c.iPage-a+1,h=f+5-1);a=0;for(iLen=d.length;a<iLen;a++){$("li:gt(0)",d[a]).filter(":not(:last)").remove();for(g=f;g<=h;g++)k=g==c.iPage+1?'class="active"':"",$("<li "+k+'><a href="#">'+g+"</a></li>").insertBefore($("li:last",d[a])[0]).bind("click",function(a){a.preventDefault();b._iDisplayStart=(parseInt($("a",this).text(),10)-1)*
