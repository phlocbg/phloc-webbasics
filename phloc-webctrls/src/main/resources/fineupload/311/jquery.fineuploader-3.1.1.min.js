...
qq.DragAndDrop=function(a){function b(){i===j&&!g&&(e.callbacks.log("Grabbed "+h.length+" files after tree traversal."),f.dropDisabled(!1),e.callbacks.dropProcessing(!1,h))}function c(a){var e;i+=1;a.isFile?a.file(function(a){h.push(a);j+=1;b()}):a.isDirectory&&(g=!0,a=a.createReader(),a.readEntries(function(a){j+=1;for(e=0;e<a.length;e+=1)c(a[e]);g=!1;a.length||b()}))}function d(a){f=new qq.UploadDropZone({element:a,onEnter:function(b){qq(a).addClass(e.classes.dropActive);b.stopPropagation()},onLeaveNotDescendants:function(){qq(a).removeClass(e.classes.dropActive)},
onDrop:function(d){e.hideDropzones&&qq(a).hide();qq(a).removeClass(e.classes.dropActive);var d=d.dataTransfer,g,l;e.callbacks.dropProcessing(!0);f.dropDisabled(!0);if(1<d.files.length&&!e.multiple)e.callbacks.error("tooManyFilesError","");else if(h=[],j=i=0,qq.isFolderDropSupported(d)){g=d.items;for(d=0;d<g.length;d+=1)if(l=g[d].webkitGetAsEntry())l.isFile?(h.push(g[d].getAsFile()),d===g.length-1&&b()):c(l)}else e.callbacks.dropProcessing(!1,d.files),f.dropDisabled(!1)}});k.addDisposer(function(){f.dispose()});
e.hideDropzones&&qq(a).hide()}var e,f,g,h=[],i=0,j=0,k=new qq.DisposeSupport;e={dropArea:null,extraDropzones:[],hideDropzones:!0,multiple:!0,classes:{dropActive:null},callbacks:{dropProcessing:function(){},error:function(){},log:function(){}}};qq.extend(e,a);return{setup:function(){e.dropArea&&e.extraDropzones.push(e.dropArea);var a,b=e.extraDropzones;for(a=0;a<b.length;a+=1)d(b[a]);e.dropArea&&(!qq.ie()||qq.ie10())&&k.attach(document,"dragenter",function(c){var d;if(d=!f.dropDisabled()){var i;qq.each(c.dataTransfer.types,
function(a,b){if("Files"===b)return i=!0,!1});d=i}if(d&&!qq(e.dropArea).hasClass(e.classes.dropDisabled)){e.dropArea.style.display="block";for(a=0;a<b.length;a+=1)b[a].style.display="block"}});k.attach(document,"dragleave",function(c){if(e.hideDropzones&&qq.FineUploader.prototype._leaving_document_out(c))for(a=0;a<b.length;a+=1)qq(b[a]).hide()});k.attach(document,"drop",function(c){if(e.hideDropzones)for(a=0;a<b.length;a+=1)qq(b[a]).hide();c.preventDefault()})},setupExtraDropzone:function(a){e.extraDropzones.push(a);
...
!c.match(/^_/)&&void 0!==b()[c]};k=function(a){var c=[],d=Array.prototype.slice.call(arguments,1);i(d,c);return b()[a].apply(b(),c)};a.fn.fineUploader=function(e){var f=this,g=arguments,h=[];this.each(function(i,m){c=a(m);if(b()&&j(e)){if(h.push(k.apply(f,g)),1===f.length)return!1}else"object"===typeof e||!e?d.apply(f,g):a.error("Method "+e+" does not exist on jQuery.fineUploader")});return 1===h.length?h[0]:1<h.length?h:this}})(jQuery);
