...
public enum ESMTPTransportProperty
{
  ...
  * InetAddress.getLocalHost().getHostName(). Should not normally need to be set
  * if your JDK and your name service are configured properly.
  */
 LOCALHOST ("localhost", String.class),

 /**
  * Local address (host name) to bind to when creating the SMTP socket. Defaults
  * to the address picked by the Socket class. Should not normally need to be
  * set, but useful with multi-homed hosts where it's important to pick a
  * particular local address to bind to.
  */
  ...
  * Normally failure of the EHLO command will fallback to the HELO command; this
  ...
  * SMTPAddressSucceededException for each address that is successful. Note also
  * that this will cause a SendFailedException to be thrown from the sendMessage
  * method of SMTPTransport even if all addresses were correct and the message
 ...
 /**
  * Specifies the port to connect to when using the specified socket factory. If
  ...
  * to false for the"smtp" protocol and true for the"smtps" protocol.
  */
  ...
  * javax.net.ssl.SSLSocketFactory class. This class will be used to create SMTP
  * SSL sockets.
  */
  ...
  * to be used if the proxy server is not using the standard port number of
  * 1080.
  */
  ...
  * set this property to false when dealing with servers that are broken in this
  * way.
  */
 NOOP_STRICT ("noop.strict", boolean.class);
                                  ...
                                  @Nonnull final Class <?> aPropertyValueClass)
  {
    ...
    this.m_aPropertyValueClass = aPropertyValueClass;
  }
  ...
  {
    return "mail.smtp." + this.m_sPropertyName;
  }
  ...
  {
    return "mail.smtps." + this.m_sPropertyName;
  }

  /**
   * @param bForSMTPS
   *        Whether or not to retrieve it for SMTPS
   * @return The property name for regular SMTP transfer. Neither
   *         <code>null</code> nor empty.
   */
  @Nonnull
